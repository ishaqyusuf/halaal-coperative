      ...
      <DropdownMenu>
        <DropdownMenuTrigger
          render={
            <Button
              ...
              className="h-8 rounded-full border-border bg-background/95 px-3 text-xs font-semibold shadow-lg backdrop-blur"
              size="sm"
              type="button"
              variant="outline"
            />
        ...
        >
          ...
          {variants.map((variant) => (
            <DropdownMenuItem
              className="cursor-pointer items-start p-2 whitespace-normal"
              key={variant.id}
              onSelect={() => {
