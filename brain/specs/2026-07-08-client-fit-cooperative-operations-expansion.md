# Spec: Client-Fit Cooperative Operations Expansion
...
40. As a tenant admin, I want procurement policy to define whether active financing blocks procurement, maximum payback months, and whether commitment can be reduced during procurement payback, so that product access and repayment behavior follow cooperative policy.
...
60. As a tenant admin, I want Foodstuff Purchase modeled as a monthly committee operation for staple items such as rice, beans, yam, garri, grains, and similar commodities, so that bulk purchase, resale/allocation, member applications, approvals, and month-end accounting are tracked properly.
61. As a member, I want to apply for Foodstuff Purchase in an active cycle, so that the committee can review my request.
62. As a Foodstuff Purchase committee staff member, I want to approve or reject member foodstuff purchase applications, so that purchases are governed before fulfillment.
63. As a Foodstuff Purchase committee staff member, I want to submit end-of-month accounting and profit for the cycle, so that the cooperative can review business performance.
64. As a finance officer, I want Foodstuff Purchase payments to support future/back periods, maximum payback months, and fixed/flexible commitment policy if applicable, so that foodstuff obligations work like real monthly commitments once posting rules are confirmed.
65. As a member, I want a portal dashboard showing profile, status, savings, shares, special savings, financing, procurement, payments, and statements, so that I can self-serve.
66. As a member, I want to submit profile or document updates for approval, so that my records can be corrected without staff retyping everything.
67. As a member, I want to see my active obligations and outstanding balances, so that I can plan payments.
68. As a member, I want notifications when receipts, financing requests, share requests, or support cases change status, so that I stay informed.
69. As a tenant admin, I want to create staff users, so that cooperative work can be delegated.
70. As a tenant admin, I want to assign staff roles, so that each staff member only has the access they need.
71. As a tenant admin, I want role permissions per module and action, so that member management, payment review, financing approval, procurement approval, share approval, settings, reports, and staff management are controlled.
72. As a tenant admin, I want super admin access separate from ordinary admin access, so that sensitive actions remain governed.
73. As an operations officer, I want member access to be limited to their own profile and records, so that members cannot see or change cooperative-wide data.
74. As a finance officer, I want approved records to be locked from casual edits, so that posted financial history remains trustworthy.
75. As a finance officer, I want corrections to use adjustment or reversal workflows, so that financial mistakes are fixed without hiding the original action.
76. As a tenant admin, I want an activity report for every important action, so that I can investigate who did what.
77. As an auditor, I want activity records to include actor, authorizer, date, affected record, and before/after values for sensitive changes, so that governance evidence is complete.
78. As a tenant admin, I want the activity report to include support resolutions, so that customer service decisions are traceable.
79. As a member, I want to open a support case, so that I can report payment mistakes or account issues.
80. As a cooperative admin, I want to respond to member support cases, so that issues are resolved inside the system.
81. As a cooperative admin, I want support cases linked to payments, receipts, shares, financing, procurement, or member records, so that context is easy to find.
82. As a cooperative admin, I want support case resolution actions to require approval when they affect money, so that service staff cannot silently change balances.
83. As a tenant admin, I want support records retained, so that disputes can be reviewed later.
84. As a tenant admin, I want exportable reports for members, shares, contributions, financing, procurement, special savings, support, and audit logs, so that we can maintain offline confidence.
85. As a tenant admin, I want backup and restore posture documented, so that the cooperative understands what happens if something goes wrong.
86. As a tenant admin, I want legal terms and conditions before pilot launch, so that both parties understand responsibilities and limitations.
87. As a platform operator, I want Sentry or equivalent error monitoring, so that code errors are reported with enough detail to debug quickly.
88. As a platform operator, I want feature requests captured and triaged, so that client feedback turns into trackable product work.
89. As a tenant admin, I want realistic reliability messaging, so that security and uptime expectations are clear instead of vague.
90. As a platform operator, I want sensitive infrastructure and error details hidden from cooperative users, so that security posture is not weakened.
91. As a tenant admin, I want hosted subdomain support and a path toward personalized cooperative domains, so that the cooperative can present a professional member-facing portal.
92. As a future member user, I want WhatsApp, email, mobile app, and AI-assisted notifications to be possible later, so that the platform can grow without redesigning core workflows.
93. As a product owner, I want unclear client-specific items captured as open questions, so that implementation does not hard-code uncertain policies.

## Implementation Decisions

- Treat this as an umbrella spec for client-fit cooperative operations, not a single small feature. Implementation should be phased, but the issue should preserve the full client context.
...
- Keep money domains separate: share capital, monthly commitment savings, special savings, financing principal, repayments, procurement obligations, project financing, Foodstuff Purchase commitments, charges, and profit/dividend allocations.
...
- Model procurement as a distinct financing product with item details, requested cost, approved cost, repayment plan, status, and policy gating. Procurement must have its own tenant configuration, separate from loan settings, covering maximum payback months and whether commitment can be reduced during procurement payback or must stay fixed.
- Model Foodstuff Purchase as a distinct monthly committee business workflow with fund release, member applications, committee approval, repayment/payment handling, and month-end accounting/profit evidence. Product-facing UI/spec language should use "Foodstuff Purchase" for staple items such as rice, beans, yam, garri, grains, and similar commodities; existing internal module keys/routes may remain `food_purchase` or `/food-purchase` until a product rename is scheduled.
- Foodstuff Purchase must have its own tenant configuration, separate from loan and procurement settings, covering maximum payback months and whether commitment can be reduced during foodstuff payback or must stay fixed.
...
- Add a staged project financing request/review layer so member business funding discussions can be captured and audited. Defer detailed project financing accounting until product semantics are confirmed, because repayable facilities, investment partnerships, and profit-sharing arrangements require different posting behavior.
...
- Procurement tests should cover item request, approval, repayment schedule creation, separate tenant policy caps, commitment-reduction policy during procurement payback, overlap with active financing where policy allows, and default/overdue reporting.
- Foodstuff Purchase tests should cover staple-item cycle setup, member application/review, separate tenant policy caps, commitment-reduction policy during foodstuff payback, receipt/payment settlement, committee accounting review, and profit evidence staying separate from member savings/share capital until a distribution workflow exists.
...
- Complete project financing accounting until the cooperative clarifies whether approved requests should post as repayable financing, investment partnerships, profit-sharing arrangements, or a mix per request.
...
- This spec synthesizes the first generated client discovery, supplemental operations/support notes, the PRD artifact, and recording-derived notes from the July 8 demo visit.
...
- Open questions to confirm before implementation include: exact charge/product short-code rule, meaning of the 200/500 values mentioned in the prelude recording, whether members can directly request additional shares, exact Foodstuff Purchase posting semantics after approval/payment, and how approved project financing should post after review.
- Assumed testing seams for this spec are: deterministic domain/package rules, database query/action helpers, typed API/router contracts, dashboard route loaders, and member/admin workflow surfaces. This follows the current repository architecture and keeps most behavior checks close to durable business state.
