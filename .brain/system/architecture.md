...
- Local named-host development is routed through `/Users/M1PRO/Documents/code/local-infra-kit` with profile `halaalvest`, standard `.env.local` / `.env.remote.local` / `.env.prod` modes, shared service readiness, and Portless workspace scripts. Local PostgreSQL follows the shared PostgreSQL contract at `127.0.0.1:55432/halaalvest`; because School Clerk uses the same host port, the two project databases cannot run concurrently. Tenant/public-site URLs use `*.halaalvest.localhost`, while local dashboard workspaces use `*.halaalvest-dash.localhost` and remain behind the HTTPS Portless entrypoint. Direct-port and path-style URLs such as `localhost:1441/<tenant>` remain explicit troubleshooting fallbacks.
