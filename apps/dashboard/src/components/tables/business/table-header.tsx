...
export function BusinessTableHeader<TData>({
  loading,
  table,
  tableScroll,
}: Props<TData>) {
  ...
  const sortableColumnIds = useMemo(() => {
    ...
    return table
      .getAllLeafColumns()
      .filter((column) => !NON_REORDERABLE_COLUMNS.business.has(column.id))
        ...
        >
          ...
          >
            {headerGroup.headers.map((header, headerIndex, headers) => {
              ...
              const canReorder =
                !NON_REORDERABLE_COLUMNS.business.has(columnId)
...
function renderHeaderContent<TData>(
  ...
  const sortField = SORT_FIELD_MAPS.business[columnId]
...
function getHeaderLabel(columnId: string): string {
  const labels: Record<string, string> = {
    ...
    business: "Business",
    capital: "Capital",
    latestProfitEntry: "Latest profit",
    period: "Period",
    profit: "Allocatable profit",
    status: "Status",
  }
