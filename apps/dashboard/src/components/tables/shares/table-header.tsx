...
export function SharesTableHeader<TData>({
  loading,
  table,
  tableScroll,
}: Props<TData>) {
  ...
  const sortableColumnIds = useMemo(() => {
    ...
    return table
      .getAllLeafColumns()
      .filter((column) => !NON_REORDERABLE_COLUMNS.shares.has(column.id))
        ...
        >
          ...
          >
            {headerGroup.headers.map((header, headerIndex, headers) => {
              ...
              const canReorder = !NON_REORDERABLE_COLUMNS.shares.has(columnId)
...
function renderHeaderContent<TData>(
  ...
  const sortField = SORT_FIELD_MAPS.shares[columnId]
