...
- During Getting Started, the Operation Profile review is presented as a guided sub-step flow instead of one dense service-access form. The flow asks about commitment collection, procurement, Foodstuff Purchase, member access, and review/save one area at a time, while the full setup rail is hidden for focus. This setup flow should not ask staff for explicit access-reduction audit reasons; post-setup settings keep that stricter audit control.
