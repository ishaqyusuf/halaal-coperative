...
- Added `@halaalvest/observability` with explicit production-plus-DSN enablement, deterministic fingerprints, original-cause reporting, bounded scalar metadata, and Halaalvest-specific member/financial redaction.
- Standardized Hono/tRPC error envelopes and request correlation, marketing API failures, dashboard/marketing/mobile error UI, TanStack Query feedback, and background retry receipts.
- Replaced dashboard raw crash audit evidence with a minimal category/code/reference/retryability/source receipt and added adoption/regression coverage preventing raw stack, path, user-agent, member, tenant, and financial content from returning.
...
- Verification at this stage: the complete repository suite passed with 811 tests; errors, observability, API, dashboard, marketing, jobs, and mobile typechecks passed; touched-file ESLint passed; and the dashboard production build passed. The marketing Turbopack build remains blocked by an existing local worker-port restriction, while its Webpack fallback reaches the existing client import of `node:crypto` in `early-access.ts`.
