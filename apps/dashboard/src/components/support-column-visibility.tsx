...
export function SupportColumnVisibility() {
  const { columns } = useSupportStore()
  ...
  return (
    <Popover>
      <PopoverTrigger
        render={
          <Button
            aria-label="Configure support columns"
            size="icon"
            variant="outline"
          />
        }
              ...
              return (
                <div className="flex items-center space-x-2" key={column.id}>
                  ...
                  <label
                    ...
                    htmlFor={`support-column-${column.id}`}
                  >
                    {label}
                  </label>
                </div>
