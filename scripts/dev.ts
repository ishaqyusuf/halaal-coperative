...
type DevProfile = "local" | "prod"
type DevTask = "dev" | "dev:portless"
...
export function parseArgs(argv: string[]): DevCliOptions {
  ...
  while (index < argv.length) {
    ...
    throw new Error(
      `Unknown dev flag: ${arg}. Use --local, --prod, or --filter/--f/-f/-filter.`
    )
  }
...
export function commandForOptions(options: DevCliOptions): string[] {
  const turboFilterArgs = buildTurboFilterArgs(options.filters)
