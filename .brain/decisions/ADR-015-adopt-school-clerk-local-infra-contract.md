...
- Use `.env.local`, `.env.dev`, `.env.preview`, and `.env.production`, with the database variable contract subsequently superseded by ADR-017.
- Use the selected mode file as the sole local connection source, currently `127.0.0.1:55434/halaalvest`, and retain the existing `halaalvest-postgres-data` volume.
...
- Missing or mode-incompatible database URL values fail closed rather than selecting an implicit database. ADR-017 defines the current profile-specific variable name.
