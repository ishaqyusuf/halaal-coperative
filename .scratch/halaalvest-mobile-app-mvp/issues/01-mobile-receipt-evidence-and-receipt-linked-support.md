# 01 — Mobile Receipt Evidence And Receipt-Linked Support

**What to build:** Members can attach mobile-safe receipt proof evidence and open a support case directly from a submitted receipt, so payment questions stay connected to finance review records.

**Blocked by:** None — can start immediately.

**Status:** done

- [x] A member can submit or update receipt proof evidence from mobile without the app owning posting or balance logic.
- [x] A member can start a support case from a receipt, and staff can see that the support case is linked to the receipt review context.
