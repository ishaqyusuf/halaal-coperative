...
- Prisma and local-infra use only the root files `.env`, `.env.local`, `.env.dev`, `.env.preview`, and `.env.production`. They load `.env` plus exactly one selected profile; the profile owns `DATABASE_URL`. Do not add aliases, package-local lookup, or cross-profile fallback to database tooling. Deployment-specific EAS credential loading remains a separate contract.
