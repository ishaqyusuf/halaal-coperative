...
export function ContributionColumnVisibility() {
  const { columns } = useContributionsTableStore()
  ...
  return (
    <Popover>
      <PopoverTrigger
        render={
          <Button
            aria-label="Configure contribution columns"
            size="icon"
            variant="outline"
          />
        }
              ...
              return (
                <div className="flex items-center space-x-2" key={column.id}>
                  ...
                  <label
                    ...
                    htmlFor={`contribution-${column.id}`}
                  >
                    {label}
                  </label>
                </div>
