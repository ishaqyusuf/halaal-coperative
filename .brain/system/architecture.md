...
- Local named-host development is supported through `portless` scripts and the shared environment bootstrap script. Workspace apps reuse the default HTTPS wildcard proxy instead of forcing a separate HTTP proxy configuration. Tenant/public-site URLs use `*.halaalvest.localhost`, while local dashboard workspaces use `*.halaalvest-dash.localhost` and remain behind the HTTPS Portless entrypoint. Direct-port and path-style URLs such as `localhost:1441/<tenant>` remain explicit troubleshooting fallbacks and do not become primary workspace or redirect targets.
