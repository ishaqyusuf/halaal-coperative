...
export function AuditTableHeader<TData>({
  loading,
  table,
  tableScroll,
}: Props<TData>) {
  ...
  const sortableColumnIds = useMemo(() => {
    ...
    return table
      .getAllLeafColumns()
      .filter((column) => !NON_REORDERABLE_COLUMNS.audit.has(column.id))
        ...
        >
          ...
          >
            {headerGroup.headers.map((header) => {
              ...
              const canReorder = !NON_REORDERABLE_COLUMNS.audit.has(columnId)
...
function renderHeaderContent<TData>(
  ...
  const sortField = SORT_FIELD_MAPS.audit[columnId]
