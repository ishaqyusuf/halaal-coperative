...
- Local named-host development is routed through `/Users/M1PRO/Documents/code/local-infra-kit` with profile `halaalvest`, standard `.env.local` / `.env.preview` / `.env.prod` modes, shared service readiness, and Portless workspace scripts. Each selected mode file's `DATABASE_URL` is authoritative; the toolkit derives transient Docker Compose settings from the local URL instead of generating a connection or consulting a port registry. Halaalvest uses `127.0.0.1:55434/halaalvest`, allowing its PostgreSQL container to run beside School Clerk on `55432`. Tenant/public-site URLs use `*.halaalvest.localhost`, while local dashboard workspaces use `*.halaalvest-dash.localhost` and remain behind the HTTPS Portless entrypoint. Direct-port and path-style URLs such as `localhost:1441/<tenant>` remain explicit troubleshooting fallbacks.
