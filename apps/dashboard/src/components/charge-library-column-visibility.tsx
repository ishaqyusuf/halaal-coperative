...
export function ChargeLibraryColumnVisibility() {
  const { columns } = useChargeLibraryTableStore()
  ...
  return (
    <Popover>
      <PopoverTrigger
        render={
          <Button
            aria-label="Configure charge library columns"
            size="icon"
            variant="outline"
          />
        }
              ...
              return (
                <div className="flex items-center space-x-2" key={column.id}>
                  ...
                  <label
                    ...
                    htmlFor={`charge-library-${column.id}`}
                  >
                    {label}
                  </label>
                </div>
