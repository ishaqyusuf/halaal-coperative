...
export function NotificationsDataTable({
  ...
  const trpc = useTRPC()
  const { filters } = useNotificationsFilterParams()
  const { params } = useSortParams()
  const parentRef = useRef<HTMLDivElement>(null)
  const { setColumns } = useNotificationTableStore()
  ...
  const infiniteQueryOptions =
    trpc.notifications.deliveryHistory.infiniteQueryOptions(queryInput, {
  ...
  if (isError) {
    return (
      <EmptyState
        ...
        title="Notification delivery history could not load."
      />
    )
  }
  ...
  if (!tableData.length) {
    return (
      <EmptyState
        description="Shared notification previews remain available above."
        title="No persisted delivery history yet."
      />
    )
  }
