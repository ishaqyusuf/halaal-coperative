...
export function FoodPurchaseTableHeader<TData>({
  loading,
  table,
  tableScroll,
}: Props<TData>) {
  ...
  const sortableColumnIds = useMemo(() => {
    ...
    return table
      .getAllLeafColumns()
      .filter((column) => !NON_REORDERABLE_COLUMNS.foodPurchase.has(column.id))
        ...
        >
          ...
          >
            {headerGroup.headers.map((header, headerIndex, headers) => {
              ...
              const canReorder =
                !NON_REORDERABLE_COLUMNS.foodPurchase.has(columnId)
...
function renderHeaderContent<TData>(
  ...
  if (columnId === "select") {
    ...
    return (
      <Checkbox
        aria-label="Select all visible Foodstuff Purchase applications"
        aria-checked={
          allPageRowsSelected
            ? true
