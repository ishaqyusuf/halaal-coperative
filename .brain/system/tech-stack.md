...
- Planned production integrations select `@sentry/bun` for the API, `@sentry/nextjs` for dashboard/marketing, `@sentry/node` plus the Sentry esbuild plugin for Trigger jobs, and `@sentry/react-native` for Expo. The dependencies, initialization, and capture hooks remain excluded until external transmission is explicitly approved and per-deployment credentials are supplied.
