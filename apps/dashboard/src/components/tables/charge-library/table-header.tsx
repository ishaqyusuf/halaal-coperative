...
export function ChargeLibraryTableHeader<TData>({
  loading,
  table,
  tableScroll,
}: Props<TData>) {
  ...
  const sortableColumnIds = useMemo(() => {
    ...
    return table
      .getAllLeafColumns()
      .filter((column) => !NON_REORDERABLE_COLUMNS.chargeLibrary.has(column.id))
        ...
        >
          ...
          >
            {headerGroup.headers.map((header, headerIndex, headers) => {
              ...
              const canReorder =
                !NON_REORDERABLE_COLUMNS.chargeLibrary.has(columnId)
...
function renderHeaderContent<TData>(
  ...
  const sortField = SORT_FIELD_MAPS.chargeLibrary[columnId]
