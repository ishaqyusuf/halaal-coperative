...
- Local named-host dev: Portless is the default via `bun run dev`; app scopes use the root dev router filter aliases such as `bun run dev -f dashboard` and `bun run dev -f web api`.
