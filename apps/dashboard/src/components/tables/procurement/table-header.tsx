...
export function ProcurementTableHeader<TData>({
  loading,
  table,
  tableScroll,
}: Props<TData>) {
  ...
  const sortableColumnIds = useMemo(() => {
    ...
    return table
      .getAllLeafColumns()
      .filter((column) => !NON_REORDERABLE_COLUMNS.procurement.has(column.id))
        ...
        >
          ...
          >
            {headerGroup.headers.map((header, headerIndex, headers) => {
              ...
              const canReorder =
                !NON_REORDERABLE_COLUMNS.procurement.has(columnId)
...
function renderHeaderContent<TData>(
  ...
  if (columnId === "select") {
    ...
    return (
      <Checkbox
        aria-label="Select all visible procurement requests"
        aria-checked={
          allPageRowsSelected
            ? true
