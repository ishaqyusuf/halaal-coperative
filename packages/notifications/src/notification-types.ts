...
>(
  ...
  return {
    ...input,
    action: input?.action ?? resolveValue(definition.defaultAction, parsed) ?? undefined,
    channels: input?.channels ?? definition.defaultChannels ?? ["in_app"],
    description:
      input?.description ?? resolveValue(definition.description, parsed) ?? undefined,
    notificationType,
    title: input?.title ?? resolveValue(definition.title, parsed) ?? "Notification",
    variant: input?.variant ?? resolveValue(definition.variant, parsed) ?? "info",
  }
}
