...
- Local named-host development is supported through `portless` scripts and the shared environment bootstrap script. Workspace apps reuse the default HTTPS wildcard proxy instead of forcing a separate HTTP proxy configuration. Tenant/public-site URLs use `*.halaalvest.localhost`; temporary dashboard aliases use `*.halaalvest-dash.localhost` and redirect to the canonical tenant host. Because that canonical local dashboard target runs directly on the plain-HTTP dashboard port, the alias redirect explicitly switches from the Portless HTTPS entrypoint to HTTP. Dashboard dev pages expose a local URL switcher for portless and path-style testing URLs such as `localhost:1441/<tenant>`.
