...
export function NotificationColumnVisibility() {
  const { columns } = useNotificationTableStore()
  ...
  return (
    <Popover>
      <PopoverTrigger
        render={
          <Button
            aria-label="Configure notification columns"
            size="icon"
            variant="outline"
          />
        }
              ...
              return (
                <div className="flex items-center space-x-2" key={column.id}>
                  ...
                  <label
                    ...
                    htmlFor={`notification-${column.id}`}
                  >
                    {label}
                  </label>
                </div>
